    ...
    var body: some View {
        GeometryReader { geometry in
            Path { path in
                ...
                let bubbleSize = CGSize(width: frameSize.width - padding, height: frameSize.height - padding)
            ...
            VStack(alignment: alignment) {
                Group {
                    Text(chatMessage.text).font(.body)
                        ...
                        .padding(20)
                        .background(
                            ChatMessageBubbleBackground(bubbleColor: chatMessage.color,
                                                        direction: direction).padding(.top, 10))
...
// swiftlint:disable line_length
    ...
    struct ChatRow_Previews: PreviewProvider {
        static var previews: some View {
            List {
                ChatRow(chatMessage: ChatMessageModel(text: "Lorem ipsum dolor sit amet, consectetur adipiscing elit, sed do eiusmod tempor incididunt ut labore et dolore magna aliqua. Ut enim ad minim veniam, quis nostrud exercitation ullamco laboris nisi ut aliquip ex ea commodo consequat.", isSentByCurrentUser: true, author: AuthorModel(avatar: "A", name: "You", identity: "A"))).frame(maxWidth: .infinity)
                ChatRow(chatMessage: ChatMessageModel(text: "Duis aute irure dolor in reprehenderit in voluptate velit esse cillum dolore eu fugiat nulla pariatur. Excepteur sint occaecat cupidatat non proident, sunt in culpa qui officia deserunt mollit anim id est laborum.", isSentByCurrentUser: false, author: AuthorModel(avatar: "B", name: "Betty", identity: "B"))).frame(maxWidth: .infinity)
